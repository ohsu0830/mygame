...
class LoveBulletAdventure:
    def __init__(self):
        ...
        # 배경음악
        bgm_path = os.path.join(MUSIC_DIR, "backgroun2.mp3")
    ...
    def get_name_screen(self):
        ...
        bg_path = os.path.join(IMG_DIR, "background111.png")
        ...
        self.start_btn = Button(self.window, text="시작!", font=("Arial",16), command=self.show_manual)
...
행운을 빌지!
"""
        self.canvas.create_text(400, 210, text=manual_part1, font=("Arial", 16), fill="deeppink", justify="left")
        self.canvas.create_text(400, 285, text=red_text1, font=("Arial", 16), fill="deeppink", justify="left")
        self.canvas.create_text(400, 310, text=red_text2, font=("Arial", 16), fill="red", justify="left")
        self.canvas.create_text(400, 405, text=manual_part2, font=("Arial", 16), fill="deeppink", justify="left")
    ...
    def start_game(self):
        ...
        # 스테이지 2 배경화면
        img2 = Image.open(os.path.join(IMG_DIR, "background2.gif"))
        img2 = img2.resize((800, 600))
        self.bg2 = ImageTk.PhotoImage(img2)

        #  엔딩 배경화면
        img3 = Image.open(os.path.join(IMG_DIR, "background3.png"))
        img3 = img3.resize((800, 600))
        self.end_bg = ImageTk.PhotoImage(img3)

        self.player_img = PhotoImage(file=os.path.join(IMG_DIR, "Girl.png"))
        self.bullet_img = PhotoImage(file=os.path.join(IMG_DIR, "heart.png"))

        # 적 이미지
        enemy_img_paths = {
            1: os.path.join(IMG_DIR, "Hat_man1.png"),
            5: os.path.join(IMG_DIR, "5s.png"),
            10: os.path.join(IMG_DIR, "10s.png"),
            50: os.path.join(IMG_DIR, "50s.png")
        }

        self.enemy_imgs = {}
        # 1점, 10점
        self.enemy_imgs[1] = PhotoImage(file=enemy_img_paths[1])
        self.enemy_imgs[10] = PhotoImage(file=enemy_img_paths[10])

        # 5점
        img5 = Image.open(enemy_img_paths[5])
        resized_img5 = img5.resize((img5.width * 2, img5.height * 2))
        self.enemy_imgs[5] = ImageTk.PhotoImage(resized_img5)

        # 50점 
        img50 = Image.open(enemy_img_paths[50])
        resized_img50 = img50.resize((int(img50.width * 1.5), int(img50.height * 1.5)))
        self.enemy_imgs[50] = ImageTk.PhotoImage(resized_img50)
    ...
    def shoot(self):
        now = time.time()
        if now - self.last_shot < 1:
            return
        self.last_shot = now

        bullet = self.canvas.create_image(self.player_x+20, self.player_y,
                                          anchor=NW, image=self.bullet_img)
        self.bullets.append(bullet)

        sound_path = os.path.join(MUSIC_DIR, "hit.mp3")
        if os.path.exists(sound_path):
            pygame.mixer.Sound(sound_path).play()

    # 적 등장
    def spawn_enemy(self, score_type):
        y = random.randint(40, 420)
        img = self.enemy_imgs[score_type]
        enemy = [self.canvas.create_image(800, y, anchor=NW, image=img),
                 800, y, score_type]
            ...
            for b in self.bullets[:]:
                if self.collide(img, b):
                    ...
                    txt = self.canvas.create_text(
                        x, y, text=random.choice(self.enemy_lines[s]),
